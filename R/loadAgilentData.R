...
loadAgilentData<-function(ifile,ofile=NULL,params=list()){
  ...
  #   reps=tolower(sapply(strsplit(sid,"_"),function(x) rev(x)[1]))
  #   fac=match(gsub("[0-9]","",reps),letcode)
  #   fac[is.na(fac)]=0
  #   reps=as.numeric(gsub("[a-z]","",reps))+fac*100
  #   metainfos$Rep=reps
  #   }
  ####################################################################################################
  ...
  annot=data.frame(Analyte=nm,PutativeName=lumetnams,IsSTD=FALSE,RT=rtmed,stringsAsFactors = F)
  newnam=oldnam=lumetnams
  
  if(params$checkNams){
    newnam=cleanMetaboNames(oldnam)$newnam
    annot$PutativeName=newnam
